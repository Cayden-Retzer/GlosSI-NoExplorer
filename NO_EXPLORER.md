...
returns a frozen position and XInput reports no input. So GlosSIWatchdog (a separate process, where
all of that is real) handles the cursor instead, like Steam itself does: controller input hides it,
mouse movement brings it back. Hiding blanks the system cursor images (`SetSystemCursor`), which is
the one thing the overlay can't override; showing reloads the user's cursors (`SPI_SETCURSORS`),
which the watchdog also does when the menu closes, when it stops, and after GlosSITarget exits.
"Menu open" is read from GlosSITarget's window: it only takes mouse input while the Steam menu is
open over the launched app. Disable per shortcut with
